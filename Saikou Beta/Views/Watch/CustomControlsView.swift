                ...
                ZStack(alignment: .bottomTrailing) {
                    ...
                    VStack {
                        ...
                        VStack {
                            ...
                            if(playerVM.duration != nil) {
                                CustomView(percentage: $playerVM.currentTime, isDragging: $playerVM.isEditingCurrentTime, total: playerVM.duration!, isMacos: $isMacos)
                                    .frame(height: 20)
                                    .frame(maxHeight: 20)
                                    .padding(.bottom, playerVM.isEditingCurrentTime ? 3 : 0 )
                            } else {
                                CustomView(percentage: Binding.constant(0.0), isDragging: Binding.constant(false), total: 1.0, isMacos: $isMacos)
                                    .frame(height: 6)
                                    .frame(maxHeight: 20)
                                    .padding(.bottom, 0)
                            }
